...
const MobileSidebar = () => {
  ...
  return (
    <div>
      <ToggleButton isOpen={isOpen} onClick={openSidebar} />
      {isOpen && (
        <div
          className=" z-50 md:hidden top-0 left-0 h-[35rem] absolute w-full  bg-white shadow-large  flex-col justify-between"
          onClick={openSidebar}
        >
          <div className="px-8 py-6 flex flex-col h-full justify-between relative">
            <div
              className="z-50 flex absolute right-5 top-4 bg-purple-400 rounded-md p-1 shadow-medium"
              onClick={openSidebar}
            >
              {!isOpen ? (
            ...
            <div>
              <div className="h-16 w-[11rem] flex items-center bg-purple-300 rounded-2xl  pl-2">
                <Image src={jobprep} width={50} height={30} />
                <p className="text-gray-100 font-bold ml-2 pr-3">JobPrepAI</p>
              </div>
              <ul className="mt-8">
            ...
            <div className="mt-5 flex justify-end items-end  mb-4 w-full">
